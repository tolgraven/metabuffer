...
T['restore-meta-view shows all hits when result set fits inside window'] = function()
  vim.cmd('enew')
  local buf = vim.api.nvim_get_current_buf()
  local lines = {}
  for i = 1, 8 do
  ...
  local win = vim.api.nvim_get_current_win()
  pcall(vim.api.nvim_win_set_height, win, 20)
  ...
  vim.api.nvim_win_call(win, function()
    vim.fn.winrestview({ topline = 40, lnum = 45, col = 0, leftcol = 0 })
  end)

  session_view['restore-meta-view!'](
    meta,
    { topline = 40, lnum = 45, col = 0, leftcol = 0 },
    { ['project-mode'] = true, ['startup-initializing'] = false, ['project-mode-starting?'] = false },
    nil
  )
