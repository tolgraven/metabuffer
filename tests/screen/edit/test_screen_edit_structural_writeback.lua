...
T['focused-results direct insert survives write accept and jump-back restore'] = H.timed_case(function()
  local root = H.make_temp_project()
  ...
  H.wait_for(function()
    return child.lua_get(string.format([[
      ...
      return s and s.meta and s.meta.buf and vim.api.nvim_get_current_buf() == %d or false
    ]], child.lua_get([[
      local s = require('metabuffer.router')['active-by-source'][_G.__meta_source_buf]
      return s and s.meta and s.meta.buf and s.meta.buf.buffer or -1
    ]])))
  end, 4000)
...
T['regular-meta focused insert survives accept back back forward cycle'] = H.timed_case(function()
  local path = child.lua_get([[
    (function()
      local path = vim.fn.tempname() .. '.txt'
      vim.fn.writefile({
