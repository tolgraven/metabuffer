...
- `events.fnl` — Standalone generic lifecycle event dispatcher. Collects handler specs from registered modules, sorts by priority, filters by `:role-filter`, pcall-dispatches. Public API: `events.send`, `events.register!`, `events.registered-events`, `events.handlers-for`, `events.set-profile!`. The bus is used bidirectionally: compat modules consume events, core subsystems emit them, and the built-in `core_events.fnl` provider fans those lifecycle events back out into statusline/preview/info/context/sign refresh hooks. All lifecycle transitions — session start/stop, startup-ready, source switches, query updates, selection changes, project bootstrap/completion, accept/cancel, insert-enter, mode switches, directive changes — should flow through the bus instead of direct cross-module UI calls.
