...
local source_pool_mod = require("metabuffer.project.source_pool")
...
M.new = function(opts)
  ...
  local pool_helpers = source_pool_mod.new({settings = settings, ["prompt-has-active-query?"] = prompt_has_active_query_3f, ["line-matches-prefilter?"] = line_matches_prefilter_3f, ["bump-content-version!"] = bump_content_version_21})
  local push_file_entry_into_pool_21 = pool_helpers["push-file-entry-into-pool!"]
  local push_file_into_pool_21 = pool_helpers["push-file-into-pool!"]
  local set_project_pool_21 = pool_helpers["set-project-pool!"]
  local enable_full_source_syntax_21 = pool_helpers["enable-full-source-syntax!"]
