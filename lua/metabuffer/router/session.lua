...
local function build_refresh_hooks(deps)
  ...
  local function _26_(session, refresh_lines)
  ...
  return {["statusline!"] = _18_, ["preview!"] = _22_, ["restore-view!"] = _24_, ["info!"] = _26_, ["context!"] = _28_, ["refresh-change-signs!"] = _30_, ["capture-sign-baseline!"] = _32_}
end
local function register_prompt_hooks_21(deps, session)
  ...
  local function _36_(s)
    local info_window = windows.info
    if (info_window and info_window["refresh-statusline!"]) then
      return info_window["refresh-statusline!"](s)
    else
      return nil
    end
  end
...
local function activate_session_ui_21(deps, session, initial_lines)
  ...
  local function schedule_layout_refresh_21()
    if (session["project-mode"] and update_info_window) then
      ...
      local function refresh_after_21(delay)
        ...
        return vim.defer_fn(_47_, delay)
      end
  ...
  run_step_21("activate-session-ui/sync-prompt-buffer-name", _50_)
  local function _51_()
    return hide_startup_cursor_21(session)
  end
  run_step_21("activate-session-ui/hide-startup-cursor", _51_)
  local function _52_()
    return vim.api.nvim_buf_set_lines(prompt_buf, 0, -1, false, initial_lines)
  end
  run_step_21("activate-session-ui/set-prompt-lines", _52_)
  local function _53_()
    return register_prompt_hooks_21(deps, session)
  end
  run_step_21("activate-session-ui/register-prompt-hooks", _53_)
  ...
  if (preview_window and preview_window["ensure-window!"]) then
    ...
    run_step_21("activate-session-ui/ensure-preview-window", _56_)
  else
  end
  if (prompt_win and vim.api.nvim_win_is_valid(prompt_win)) then
    ...
    run_step_21("activate-session-ui/initial-prompt-layout", _58_)
  else
  end
  ...
  return vim.defer_fn(_69_, prompt_enter_duration_ms())
end
local function finish_session_startup_21(deps, curr, session, initial_query_active)
  ...
  local function schedule_single_file_info_phases_21()
    if not session["project-mode"] then
      ...
      return vim.defer_fn(_75_, (session["startup-ui-delay-ms"] or 320))
    else
      return nil
    end
  end
  local _78_
  if session["project-mode"] then
    _78_ = "finish-session-startup!/apply-minimal-source-set"
  else
    _78_ = "finish-session-startup!/apply-source-set"
  end
  local function _80_()
    if session["project-mode"] then
  ...
  run_step_21(_78_, _80_)
  curr["status-win"] = curr.win
  local function _82_()
    return events.send("on-win-create!", {win = curr.win.window, role = "main"})
  end
  run_step_21("finish-session-startup!/disable-airline", _82_)
  local function _83_()
    return curr["on-init"]()
  end
  run_step_21("finish-session-startup!/on-init", _83_)
  local function _84_()
    return events.send("on-session-ready!", {session = session, ["capture-sign-baseline?"] = true, ["refresh-lines"] = false})
  end
  run_step_21("finish-session-startup!/capture-sign-baseline", _84_)
  if (session["project-mode"] and not startup_layout_unsettled_3f) then
    local function _85_()
      return events.send("on-session-ready!", {session = session, ["refresh-lines"] = true, ["restore-view?"] = true})
    end
    run_step_21("finish-session-startup!/restore-meta-view-project", _85_)
  else
  end
  if not (session["project-mode"] and not initial_query_active) then
    ...
    run_step_21("finish-session-startup!/apply-prompt-lines", _87_)
  else
  end
  if (not session["project-mode"] and not startup_layout_unsettled_3f) then
    ...
    run_step_21("finish-session-startup!/restore-meta-view-regular", _89_)
  else
  end
  local function _91_()
    return events.send("on-session-ready!", {session = session, ["refresh-lines"] = true})
  end
  run_step_21("finish-session-startup!/emit-session-ready", _91_)
  if session["project-mode"] then
    local function _92_()
      if startup_live_3f() then
        return events.send("on-session-ready!", {session = session, ["refresh-lines"] = true})
    ...
    vim.defer_fn(_92_, (session["startup-ui-delay-ms"] or 350))
  else
  end
  ...
  local function _95_()
    if startup_live_3f() then
      ...
      vim.defer_fn(_97_, (session["startup-ui-delay-ms"] or 320))
