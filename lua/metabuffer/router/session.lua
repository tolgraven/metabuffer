...
local function finish_session_startup_21(deps, curr, session, initial_query_active)
  ...
  if not (session["project-mode"] and not initial_query_active) then
    ...
    run_step_21("finish-session-startup!/apply-prompt-lines", _84_)
  else
  end
  local function _86_()
  ...
  run_step_21("finish-session-startup!/emit-session-ready", _86_)
  if session["project-mode"] then
    local function _87_()
      if startup_live_3f() then
        return events.send("on-session-ready!", {session = session, ["refresh-lines"] = true})
    ...
    vim.defer_fn(_87_, (session["startup-ui-delay-ms"] or 350))
  else
  end
  ...
  local function _90_()
    if startup_live_3f() then
      ...
      vim.defer_fn(_92_, (session["startup-ui-delay-ms"] or 320))
