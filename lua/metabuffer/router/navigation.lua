...
M["scroll-main!"] = function(deps, prompt_buf, action)
  ...
  if (session and vim.api.nvim_win_is_valid(session.meta.win.window)) then
    ...
    local function _12_()
      ...
      vim.api.nvim_win_call(session.meta.win.window, _13_)
...
M["maybe-sync-from-main!"] = function(deps, session, force_refresh)
  ...
  return session_view["maybe-sync-from-main!"](session, force_refresh, {["active-by-prompt"] = active_by_prompt, ["schedule-source-syntax-refresh!"] = _18_, ["update-info-window"] = update_info_window})
end
M["schedule-scroll-sync!"] = function(deps, session)
  local scroll_sync_debounce_ms = deps["scroll-sync-debounce-ms"]
  local session_view = deps["session-view"]
  local function _19_(s, force_refresh)
    return M["maybe-sync-from-main!"](deps, s, force_refresh)
  end
  return session_view["schedule-scroll-sync!"](session, {["scroll-sync-debounce-ms"] = scroll_sync_debounce_ms, ["maybe-sync-from-main!"] = _19_})
end
return M
