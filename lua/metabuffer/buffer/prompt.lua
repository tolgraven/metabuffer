...
local function control_token_style(option_prefix, tok)
  local token = (tok or "")
  ...
  if (escaped_prefix_3f or not matches_3f) then
    ...
    local _3_
    if off_3f then
      _3_ = "MetaPromptFlagHashOff"
    else
      _3_ = "MetaPromptFlagHashOn"
    end
    local _5_
    if functional_3f then
      if off_3f then
        _5_ = "MetaPromptFlagTextFuncOff"
      else
        _5_ = "MetaPromptFlagTextFuncOn"
      end
    else
      if off_3f then
        _5_ = "MetaPromptFlagTextOff"
      else
        _5_ = "MetaPromptFlagTextOn"
      end
    end
    return {["hash-hl"] = _3_, ["text-hl"] = _5_}
  end
end
...
local function prompt_tokens(txt)
  local or_10_ = query_mod["tokenize-line"]
  if not or_10_ then
...
local function directive_arg_style(option_prefix, tok)
  local token = (tok or "")
  local parsed = directive_mod["parse-token"](option_prefix(), token)
...
local function inline_file_filter_style(option_prefix, tok)
  local token = (tok or "")
  ...
  if colon then
    ...
    local style = (parsed and control_token_style(option_prefix, flag_token))
...
M["highlight-like-line!"] = function(buf, ns, row, txt, primary_hl, option_prefix)
  ...
  for _, token in ipairs(tokens) do
    ...
    if (s and e) then
      ...
      do
        local val_111_auto = inline_file_filter_style(option_prefix, token)
        if val_111_auto then
          ...
          local val_110_auto = control_token_style(option_prefix, token)
      ...
      await_style = directive_arg_style(option_prefix, token)
...
local function render_project_flags_footer_21(session, session_prompt_valid_3f, schedule_loading_indicator_21)
...
M["refresh-highlights!"] = function(session, opts)
  local _let_29_ = (opts or {})
  local option_prefix = _let_29_["option-prefix"]
  local session_prompt_valid_3f = _let_29_["session-prompt-valid?"]
  local schedule_loading_indicator_21 = _let_29_["schedule-loading-indicator!"]
  if (session["prompt-buf"] and vim.api.nvim_buf_is_valid(session["prompt-buf"])) then
    ...
    for row, line in ipairs((lines or {})) do
      ...
      M["highlight-like-line!"](session["prompt-buf"], ns, r, txt, primary_hl, option_prefix)
    end
    return render_project_flags_footer_21(session, session_prompt_valid_3f, schedule_loading_indicator_21)
  else
    return nil
  end
end
