...
M["apply-router-defaults"] = function(router, vim)
  ...
  if (vim.g.meta_main_keymaps == nil) then
    vim.g.meta_main_keymaps = vim.deepcopy(default_main_keymaps)
  else
  end
  if (vim.g.meta_prompt_fallback_keymaps == nil) then
    vim.g.meta_prompt_fallback_keymaps = vim.deepcopy(default_prompt_fallback_keymaps)
  else
  end
