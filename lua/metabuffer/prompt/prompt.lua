...
M.new = function(nvim)
  ...
  self["on-keypress"] = function(keystroke)
    ...
    if is_action_keystroke(s) then
      ...
      return self.action.call(self, action)
    else
      return self["update-text"](s)
    end
  end
  ...
  self.start = function()
    ...
    if vim.o.timeout then
      timeoutlen = (vim.o.timeoutlen / 1000.0)
    else
      timeoutlen = nil
    end
    ...
    local function _5_()
      if ((e == "Keyboard interrupt") or string.find(tostring(e), "Keyboard interrupt")) then
