...
local function _getcode(timeoutlen, callback, interval)
  ...
  local timeout_ms = (timeoutlen and math.floor((timeoutlen * 1000)))
  local done = false
  local out = nil
  while not done do
    if callback then
    ...
    if (code ~= 0) then
      ...
      if not done then
        vim.wait(math.floor(((interval or 0.033) * 1000)))
      else
      end
    end
  end
...
M.new = function()
  ...
  self.harvest = function(nvim, timeoutlen, callback, interval)
    ...
    while not resolved do
      ...
      if ((code == nil) and (previous == nil)) then
      else
        if (code == nil) then
          resolved = (self.resolve(nvim, previous, true) or previous)
        else
          local k = key_mod.parse(nvim, code)
          if previous then
