...
local function __3eset(tbl)
  if tbl then
    assert(table_3f(tbl), "Table required as input to ->set.")
    local result = {}
    for _n, v in ipairs(tbl) do
      result[v] = true
    end
    return result
  else
...
local function contains_3f(tbl, v)
  if tbl then
    assert(table_3f(tbl), "contains? expects a table")
    if sequential_3f(tbl) then
      local function _72_(_241)
        return (_241 == v)
      end
      return (some(_72_, tbl) or false)
    else
      return (true == tbl[v])
    end
  else
