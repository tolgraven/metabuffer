...
local function delete_match_21(id, win)
  if (win and vim.api.nvim_win_is_valid(win)) then
    local or_3_ = pcall(vim.fn.matchdelete, id, win)
    if not or_3_ then
      local function _4_()
        return vim.fn.matchdelete(id)
      end
      or_3_ = pcall(vim.api.nvim_win_call, win, _4_)
    end
    return or_3_
  else
...
local function matchadd_in_window(group, pattern, win)
  if (win and vim.api.nvim_win_is_valid(win)) then
    ...
    if ok then
      ...
      return vim.api.nvim_win_call(win, _6_)
    end
  else
    return vim.fn.matchadd(group, pattern, M["default-match-priority"])
  end
end
local function remove_highlight_21(self)
  for _, id in ipairs((self["match-ids"] or {})) do
    delete_match_21(id, self["match-win"])
  end
  self["match-ids"] = {}
  if self["char-match-id"] then
    delete_match_21(self["char-match-id"], self["match-win"])
...
local function highlight_query_21(self, query, ignorecase, target_win)
  remove_highlight_21(self)
  if (query and (query ~= "")) then
      ...
      for idx, item_query in ipairs(query) do
        ...
        if (type(item_pat) == "string") then
          ...
          if (type(item_pat) == "table") then
            for _, item in ipairs(item_pat) do
    ...
    if self["also-highlight-per-char"] then
      local _20_
      if (type(query) == "table") then
        _20_ = table.concat(query, " ")
      else
        _20_ = query
      end
      self["char-match-id"] = matchadd_in_window(M["default-hi-char"], table.concat(vim.split(_20_, ""), "\\|"), win)
      return nil
    else
      return nil
    end
...
M.new = function(name, opts)
  local self
  local or_24_ = (opts and opts["get-highlight-pattern"])
  if not or_24_ then
  ...
  local or_26_ = (opts and opts.filter)
  if not or_26_ then
    local function _27_(_, _0, _1, _2)
      return {}
    end
    or_26_ = _27_
  end
  self = {name = name, ["also-highlight-per-char"] = (opts and opts["also-highlight-per-char"]), ["match-ids"] = {}, ["char-match-id"] = nil, ["match-win"] = nil, ["get-highlight-pattern"] = or_24_, filter = or_26_}
  self["remove-highlight"] = function()
    return remove_highlight_21(self)
  end
  self.highlight = function(_, query, ignorecase, target_win)
    return highlight_query_21(self, query, ignorecase, target_win)
  end
  return self
end
