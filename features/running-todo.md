...
- 74 [x] popup window for completion information doesn't update contents when you select a new option with <Tab>
...
- 79 [] prompt completion popup (info above prompt) doesn't disappear when pressing <CR> to go to a hit, and sometimes just when having finished typing something like `#exp:fn`.
- 80 [] when resizing nvim window (tmux pane split width or font size change) the preview split doesn't adjust, get either nothing or garbage on the right of it instead.
- 81 [x] when scrolling quickly enough that info window lookahead can't keep up, refresh appears to occur from 0 instead of incrementally
- 82 [] when exiting metabuffer with <CR> or <Esc> my airline shows the correct mode (N) but is stuck showing insert mode color
- 83 [x] investigate whether it would be beneficial to decouple events.send from the actual running of handlers, so that events get posted onto a true event bus that can then do things like warn about duplicate event fires within same tick, chunk the running of handlers across time if any of them are slow (essentially, yield between each fire), cancel scheduled fires in case a superceding event comes in (to exit for example)
- 84 [x] loading indicator animation ("Waiting" in main statusbar on the left side) gone from project view when loading. 
