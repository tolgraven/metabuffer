...
- 81 [] when scrolling quickly enough that info window lookahead can't keep up, refresh appears to occur from 0 instead of incrementally
...
- 85 [x] a number of animations we used to have are now gone. For example info window fading in opacity, scroll (<C-d> etc) animations. Only the animated appearance of the prompt/preview splits are still in place.
